...
const getAccessProfileByName = async (apiConfig, accessProfileName) => {
    ...
    const accessProfile =
        accessProfileResponse.data.length == 1
            ? accessProfileResponse.data[0]
            : null;
...
const exportAccessProfiles = async (apiConfig) => {
    ...
    for (const accessProfile of accessProfiles.data) {
        winston.info(
            `Exporting Access Profile: ${accessProfile.name} (${accessProfile.id})`,
        );
        ...
        if (
            accessProfile.accessRequestConfig != null &&
            accessProfile.accessRequestConfig.approvalSchemes != null
        ) {
            for (const scheme of accessProfile.accessRequestConfig
                .approvalSchemes) {
        ...
        if (
            ...
            for (const scheme of accessProfile.revocationRequestConfig
                .approvalSchemes) {
                switch (scheme.approverType) {
                    case 'GOVERNANCE_GROUP':
                        const govGroup = await getGovGroupById(
...
const migrateAccessProfile = async (apiConfig, accessProfileJson) => {
    ...
    const source = await getSourceByName(
        apiConfig,
        localAccessProfile.source.name,
    );
    ...
    if (
        ...
        for (const scheme of localAccessProfile.accessRequestConfig
            .approvalSchemes) {
            switch (scheme.approverType) {
                case 'GOVERNANCE_GROUP':
    ...
    if (
        ...
        for (const scheme of localAccessProfile.revocationRequestConfig
            .approvalSchemes) {
            switch (scheme.approverType) {
                case 'GOVERNANCE_GROUP':
                    const govGroup = await getGovGroupByName(
    ...
    let currentTargetAccessProfile =
        currentAccessProfileResponse.data.length == 1
            ? currentAccessProfileResponse.data[0]
            : null;

    if (!currentTargetAccessProfile) {
        winston.info(`Creating new access profile: ${localAccessProfile.name}`);
        try {
            const createAccessProfileResponse =
                await accessProfilesApi.createAccessProfile({
        ...
        for (const key of existingAttributeToKeep) {
            _.set(
                localAccessProfile,
                key,
                _.get(currentTargetAccessProfile, key),
            );
        }
