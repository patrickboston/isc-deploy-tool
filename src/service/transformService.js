...
const exportTransforms = async (apiConfig) => {
    ...
    const transforms = await Paginator.paginate(
        transformsApi,
    ...
    for (const transform of transforms.data) {
        ...
        if (!transform.internal) {
            winston.info(
                `Exporting Transform: ${transform.name} (${transform.id})`,
            );
...
const migrateTransform = async (apiConfig, transformJson) => {
    ...
    let currentTargetTransform =
        currentTransformResponse.data.length == 1
            ? currentTransformResponse.data[0]
            : null;
    ...
    if (!currentTargetTransform) {
        ...
        winston.info(
            `Updating existing transform: ${currentTargetTransform.name} (${currentTargetTransform.id})`,
        );
        ...
        for (const transformKey of existingAttributeToKeep) {
            _.set(
                localTransform,
                transformKey,
                _.get(currentTargetTransform, transformKey),
            );
        }
